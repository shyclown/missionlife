...
class File
{
  ...
  public function number_of_rows(){
    $sql = 'SELECT COUNT(*) AS `count` FROM `missionlife`.`ml_file`';
    return $this->db->query($sql);
  }
  public function select_all($data){
    $sql = "SELECT * FROM `missionlife`.`ml_file`";
    return $this->db->query($sql);
  }
  public function load_images($data){
    $sql = "SELECT * FROM `missionlife`.`ml_file` WHERE `file_type` = 'png'";
    return $this->db->query($sql);
  }
  ...
  public function insert_new_file($data){
    $sql = "INSERT INTO `missionlife`.`ml_file` (`id`, `file_name`,`file_type`, `file_size`, `file_src`, `date_created`,`date_edited`)
  ...
  public function attach_to_garant($data){
    $sql = "INSERT INTO `missionlife`.`ml_garant_file` (`id`, `garant_id`, `file_id`)
            VALUES (NULL, ?, ?)";
    $params = array( 'ii', $data['garant_id'], $data['file_id']);
    return $this->db->query($sql, $params);
  }
  public function attach_to_article($data){
    $sql = "INSERT INTO `missionlife`.`ml_article_file` (`id`, `article_id`, `file_id`, `file_desc`)
            VALUES (NULL, ?, ?, '')";
  ...
  public function update_file($data){
    $sql = "UPDATE `missionlife`.`ml_file` SET `file_name`=?, `file_desc`=?, `date_edited`=CURRENT_TIMESTAMP  WHERE `id` = ?";
    $params = array('ssi', $data['file_name'], $data['file_desc'], $data['file_id']);
    return $this->db->query($sql,$params);
  }
  public function update_file_desc_in_article($data){
    $sql = "UPDATE  `missionlife`.`ml_article_file` SET  `file_desc` =  ? WHERE  `ml_article_file`.`article_id` = ? AND  `ml_article_file`.`file_id` = ?";
    $params = array('sii', $data['file_desc'], $data['article_id'], $data['file_id'] );
    return $this->db->query($sql,$params);
  }
  // remove
  public function remove_from_garant($data){
    $sql = "DELETE FROM `missionlife`.`ml_garant_file`
